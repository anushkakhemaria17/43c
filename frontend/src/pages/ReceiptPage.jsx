...
import { Download, ChevronLeft, Calendar, User, Clock, Monitor, Printer, ShieldCheck } from 'lucide-react';
...
const ReceiptPage = () => {
    ...
    const fetchBooking = async () => {
        try {
            ...
            const item = res.data.find(b => b.booking_id === id);
            setBooking(item);
        } catch (err) {
            console.error(err);
        } finally {
    ...
    const qrData = JSON.stringify({
        ...
        v: "1.0",
        url: `${window.location.origin}/admin/verify-qr/${booking.booking_id}`
    });
    ...
    return (
        <div className="pt-32 pb-20 px-4 max-w-xl mx-auto min-h-screen">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                ...
                <div id="digital-pass" ref={printRef} className="relative group">
                    {/* Decorative Elements */}
                    ...
                    <div className="glass-card overflow-hidden !rounded-[2.5rem] shadow-2xl relative">
                        ...
                        <div className="p-10 space-y-12 bg-[#05071A]/50 relative">
                            <div className="flex justify-center flex-col items-center gap-6">
                                <div className="p-4 bg-white rounded-[2rem] shadow-[0_0_30px_rgba(255,255,255,0.1)] transition-transform hover:scale-105 duration-500">
                ...
                <div className="mt-12 flex flex-col sm:flex-row gap-4 no-print">
                    <button 
                        onClick={handlePrint}
                        className="gold-button flex-1 flex items-center justify-center gap-4 group"
                    >
                ...
                <p className="mt-12 text-center text-white/20 text-[10px] leading-loose uppercase tracking-[0.2em] max-w-xs mx-auto no-print">
                    Access is strictly limited to the confirmed slot. Present this pass to the concierge upon arrival.
                </p>
            </motion.div>
            ...
            <style dangerouslySetInnerHTML={{ __html: `
                @media print {
                    ...
                    #digital-pass { margin: 0 !important; border: 1px solid #eee !important; color: black !important; }
                    .glass-card { background: white !important; color: black !important; border: none !important; box-shadow: none !important; }
                    .gold-text-gradient { color: #000 !important; background: none !important; -webkit-text-fill-color: initial !important; }
                    .bg-gradient-to-br { background: #f9f9f9 !important; }
                    .bg-[#05071A] { background: white !important; }
                    .text-white\\/20, .text-white\\/40 { color: #888 !important; }
                    .text-accent { color: #b58d4a !important; }
                }
            ` }} />
        </div>
    );
