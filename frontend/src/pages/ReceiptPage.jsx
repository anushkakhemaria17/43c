...
import { formatSlotsDisplay, SLOT_HOURS } from '../utils/slots';
  ...
  return (
    <div className="pt-32 pb-20 px-4 max-w-xl mx-auto min-h-screen">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <div className="p-10 space-y-12 bg-[#05071A]/50 relative">
              ...
              <div className="grid grid-cols-2 gap-y-10 border-t border-b border-white/5 py-10">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-lg font-heading tracking-tight">{new Date(booking.date + 'T00:00:00').toLocaleDateString('en', { month: 'short', day: 'numeric', year: 'numeric' })}</p>
                </div>
                <div className="space-y-1 text-right">
                  <p className="text-white/20 uppercase text-[9px] tracking-widest font-black">Times</p>
                  <p className="text-sm font-heading tracking-tight text-accent">{booking.screen||'Screen 1'} • {formatSlotsDisplay(booking.slots)}</p>
                </div>
              </div>

              <div className="flex justify-between items-center px-2">
                <div className={`flex items-center gap-2 text-[10px] uppercase font-black tracking-widest px-4 py-2 rounded-full border 
                  ${booking.payment_status === 'completed' || booking.payment_status === 'confirmed' ? 'text-green-500/80 bg-green-500/5 border-green-500/20' : 'text-red-500/80 bg-red-500/5 border-red-500/20'}`}>
                  <ShieldCheck size={14} /> {booking.payment_status}
                </div>
        ...
        <div className="mt-12 flex flex-col sm:flex-row gap-4 no-print flex-wrap">
